...
| T58 | x | API-key admin bypass closed: `assertAdminUser` (owner/admin JWT only) on member + API-key management routes; `validateTenantHook` verifies `params.id === apiKey.tenantId`; company name/logo opened to `pengaturan` scope; backend scope allowlist + fixed API-key scope enum | V35,V36,V37,B28,B29,B30 |
