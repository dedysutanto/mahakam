...
| T58 | x | API-key admin bypass closed: `assertAdminUser` (owner/admin JWT only) on member + API-key management routes; `validateTenantHook` verifies `params.id === apiKey.tenantId`; company name/logo opened to `pengaturan` scope; backend scope allowlist + fixed API-key scope enum; mobile sticky columns narrowed to checkbox + No. Faktur only | V35,V36,V37,B28,B29,B30 |
...
| B30 | 2026-08-27 | API-key scope enum was stale/wrong (`'suplier'/'biaya'/'pelaporan'/'pengguna'` bogus; `penawaran/pengeluaran/pajak/laporan` missing) → Fastify validation rejected legit scopes; member scope arrays accepted arbitrary strings | Scope allowlist enforced in backend (`sanitizeScopes`); API-key enum matches real keys minus `pengaturan` (V37) |
