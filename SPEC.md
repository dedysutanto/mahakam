...
- Per-menu scopes on TenantUser/ApiKey (`scopes String[]`): buku-besar, faktur, penawaran, pembelian, pengeluaran, produk, pelanggan, pajak, laporan, pengaturan. Owner/admin = implicit ALL scopes. Staff/API keys must have explicit scopes; backend enforces per route module on writes and on financial reads (reports, ledgers, invoices, quotes, purchases, expenses); reference-data reads (products, customers, taxes, settings) stay open to any authenticated tenant member because cross-module forms depend on them; dashboard is the post-login landing page and is open to all members. `pengaturan` scope grants editing of company settings AND company profile (name, logo). Member management and API-key management are admin-only (owner/admin JWT only — API keys never pass). Frontend hides menus without scope.
...
- V35: API keys are NEVER tenant admins — `assertAdminUser` rejects `userId === 'api-key'`; member management (Pengguna & Akses) and API-key management are admin-only for JWT users. API keys also cannot access another tenant via `:id` routes — `validateTenantHook` verifies `params.id === apiKey.tenantId` when `fromParams` is set.
- V36: Company profile (name, logo) and settings are editable by anyone holding the `pengaturan` scope — staff with settings access or admins; `PUT /api/tenants/:id` and `POST /api/tenants/:id/logo` require `pengaturan` scope, not admin role.
- V37: Scope strings are validated against a backend allowlist (`buku-besar, faktur, penawaran, pembelian, pengeluaran, produk, pelanggan, pajak, laporan, pengaturan`) on member create/update and API-key create; API keys cannot be granted `pengaturan`.
- V38: Document numbering uses format templates with tokens (`{000}` = sequence, `{YYYY}` = year, `{MM}`/`{RM}` = month, `{DD}` = day). Monthly reset happens implicitly when `{MM}` or `{RM}` is included in the format. Sequence can be anywhere in the template (start, middle, end). Backward compatible: existing `prefix/year/digits` settings are auto-converted to the legacy format `{PREFIX}-{YYYY}-{0000}`.
...
| T58 | x | API-key admin bypass closed: `assertAdminUser` (owner/admin JWT only) on member + API-key management routes; `validateTenantHook` verifies `params.id === apiKey.tenantId`; company name/logo opened to `pengaturan` scope; backend scope allowlist + fixed API-key scope enum; mobile sticky columns narrowed to checkbox + No. Faktur only; long invoice numbers truncated with ellipsis | V35,V36,V37,B28,B29,B30,B31,B32 |
| T59 | x | Flexible document numbering: format templates with `{000}`/`{SEQ}`, `{YYYY}`, `{YY}`, `{MM}`, `{RM}` (Roman), `{DD}` tokens; sequence can be anywhere in template; monthly reset via `{MM}`/`{RM}`; backward compat with legacy prefix/year/digits settings; new Settings UI with single format input + token reference + live preview | V38,B33 |
...
| B28 | 2026-08-27 | Any API key was a full tenant admin: `authHook` sets `role:'admin'` for API keys, member/API-key routes gated on `isAdminRole(request.user.role)` passed trivially, and `validateTenantHook`'s API-key branch ignored the `:id` param → an API key could add admins to ANY tenant (cross-tenant takeover), create/delete API keys, reset passwords, rename companies | `assertAdminUser` (owner/admin JWT only, never API keys) on member/API-key routes; `validateTenantHook` verifies `params.id === apiKey.tenantId` when `fromParams` is set (V35) |
| B29 | 2026-08-27 | Staff with `pengaturan` scope could not update company name or logo — backend gated `PUT /:id` and `POST /:id/logo` by admin role while the frontend showed both sections (save always failed) | Both routes now require `pengaturan` scope, so settings-scoped staff (and admins) can update company profile (V36) |
| B30 | 2026-08-28 | Mobile sticky columns on Invoices table locked Pelanggan column — scrolled table was too narrow to read because three sticky columns (checkbox, No. Faktur, Pelanggan) consumed too much of the visible width | Removed sticky from Pelanggan header and cell; only checkbox + No. Faktur remain locked on mobile |
| B31 | 2026-08-27 | API-key scope enum was stale/wrong (`'suplier'/'biaya'/'pelaporan'/'pengguna'` bogus; `penawaran/pengeluaran/pajak/laporan` missing) → Fastify validation rejected legit scopes; member scope arrays accepted arbitrary strings | Scope allowlist enforced in backend (`sanitizeScopes`); API-key enum matches real keys minus `pengaturan` (V37) |
| B32 | 2026-08-28 | Long invoice numbers (e.g. `020/INVOICE/OSB/VII/2026/000001`) overflowed the 144px sticky No. Faktur cell on mobile, overlapping into the Pelanggan column — the `<td>` had `max-w-[144px]` but no `overflow-hidden`, and the `<span>` had no `truncate` | Added `overflow-hidden` to `<th>` and `<td>`, `truncate block` to `<span>` — long numbers now clip with ellipsis |
| B33 | 2026-08-28 | Document numbering was inflexible — fixed `{prefix}-{YYYY}-{digits}` format (e.g. `INV-2026-0001`); no option for Roman numeral months, monthly resets, or custom patterns | Format template system with tokens `{000}`, `{SEQ}`, `{YYYY}`, `{YY}`, `{MM}`, `{RM}`, `{DD}`; sequence can be anywhere; monthly reset via `{MM}`/`{RM}` in format; backward compat with legacy settings (V38) |
| B34 | 2026-08-28 | No company abbreviation field — `{ABBR}` token in numbering formats had no source; users who wanted abbreviations in document numbers had no way to set them | `company_abbreviation` setting (3-char max) with `{ABBR}` token in numbering engine; smart suggestion from company name; optional (empty → empty string) (V39) |
| B35 | 2026-08-28 | Invoice items required a catalog product — no way to add arbitrary charges like "Ongkir", "Biaya Admin", etc. as line items; users had to create dummy products for non-product charges | "Tambah Item Non Product" button adds manual items (`productId: null`); free-text input for name/price; per-row mode switching via 📦/✕ buttons; same grid and PDF rendering as product items (V40) |
| B36 | 2026-08-28 | Non-product invoice items had no unit — the PDF rendered empty string after quantity because `InvoiceItem` had no `unit` field and `item.product?.unit` was null for manual items | Added `unit VARCHAR(20)` to InvoiceItem schema; backend auto-fills from Product.unit or accepts from request body; PDF renders `item.unit \|\| item.product?.unit`; frontend shows unit dropdown for manual items (V41) |
| B37 | 2026-08-28 | Numbering engine filtered by exact rendered suffix (`endsWith: '/INV/MS/VIII/2026'`); when format or ABBR changed, old invoices with different suffixes were invisible → sequence always started at 1 | Replaced suffix filter with date-field filter (`issueDate`/`orderDate`/`date` matching monthly/yearly context); sequence extracted via regex on token pattern instead of fixed slice (V42) |
